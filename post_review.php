    ...
    <main class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                echo '<p class="error-detail">エラー詳細: ' . htmlspecialchars($e->getMessage()) . '</p>';
                echo '</div>';
            }
        }
                    ...
                    <select name="student_id" id="student_id" required>
                        ...
                        try {
                            ...
                            foreach ($students as $student) {
                                $gradeText = $student['grade'] == 5 ? '院生' : $student['grade'] . '年';
                                $selected = (isset($_POST['student_id']) && $_POST['student_id'] == $student['id']) ? 'selected' : '';
                                echo "<option value=\"{$student['id']}\" $selected>{$student['name']} ({$student['university']} {$student['department']} {$gradeText})</option>";
                            }
                        } catch (Exception $e) {
